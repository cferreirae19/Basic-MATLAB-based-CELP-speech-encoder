...
s_tr = s.'; 
nframes = floor(length(s_tr)/Ltrama);
s_tr = s_tr(1:nframes*Ltrama);
...
for k=1:length(s_tr)/Ltrama
    ...
    for i=1:4   % Para cada una de las 4 subtramas que conforman 1 trama
        ...
        for t=1:(2*Lsubtrama+1)
            ...
            y20t_tr = y20t.';
            bt = (s_tr(((k-1)*Ltrama+(i-1)*Lsubtrama+1):((k-1)*Ltrama+i*Lsubtrama))*y20t_tr)/(y20t*y20t_tr);   % Cálculo de la ganancia
            if ~isfinite(bt)
                bt = 1;
            end
        ...
        for l=1:length(v)
            ...
            y10l_tr = y10l.';
            gl = (u0_def*y10l_tr)/(y10l*y10l_tr);    % Cálculo de la ganancia
            if ~isfinite(gl)
                gl = 1;
            end
        ...
        d = max_gain_g*v(max_index_l) + max_gain_b*adapt(max_index_t:(max_index_t+Lsubtrama-1));
        adapt_1_new = adapt((Lsubtrama+1):2*Lsubtrama);
        adapt_2_new = adapt((2*Lsubtrama+1):3*Lsubtrama);
        adapt = [adapt_1_new;adapt_2_new;d];

        % Calculamos la subtrama correspondiente para la señal reconstruida.
